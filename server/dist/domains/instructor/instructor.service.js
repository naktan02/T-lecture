...
const instructor_repository_1 = __importDefault(require("./instructor.repository"));
...
class InstructorService {
    ...
    async getAvailabilities(instructorId, year, month) {
        ...
        const availabilities = await instructor_repository_1.default.findAvailabilities(instructorId, startDate, endDate);
    ...
    async updateAvailabilities(instructorId, year, month, newDatesStr) {
        ...
        const activeAssignmentDates = await instructor_repository_1.default.findActiveAssignmentsDate(instructorId, startDate, endDate);
        // 유효성 검사: 배정된 날짜가 newDatesStr에 모두 포함되어 있는지 확인
        const assignedDatesSet = new Set(activeAssignmentDates
            .filter((d) => d !== null)
            .map((d) => d.toISOString().split('T')[0]));
        const newDatesSet = new Set(newDatesStr);
        for (const assignedDate of assignedDatesSet) {
            if (!newDatesSet.has(assignedDate)) {
                throw new AppError_1.default(`이미 배정이 확정된 날짜(${assignedDate})는 근무 가능일에서 제외할 수 없습니다.`, 409, 'AVAILABILITY_CONFLICT', { assignedDate });
            }
        }
        // 업데이트 수행
        await instructor_repository_1.default.replaceAvailabilities(instructorId, startDate, endDate, newDatesStr);
        return { message: '근무 가능일이 저장되었습니다.' };
    }
    // 통계 조회
    async getInstructorStats(instructorId) {
        const [assignmentCount, rawAssignments, legacyStats] = await Promise.all([
            instructor_repository_1.default.countActiveAssignments(instructorId),
            instructor_repository_1.default.findAssignmentsForCalc(instructorId),
            instructor_repository_1.default.findLegacyStats(instructorId),
        ]);
    ...
    async updateVirtues(instructorId, virtueIds) {
        await instructor_repository_1.default.updateVirtues(instructorId, virtueIds);
    ...
    async requestPromotion(instructorId, desiredLevel) {
        ...
        const MIN_HOURS_FOR_PROMOTION = constants_1.PROMOTION_CRITERIA.MIN_LECTURE_HOURS;
        if (stats.lectureHours < MIN_HOURS_FOR_PROMOTION) {
            throw new AppError_1.default(`승급 신청 자격이 부족합니다. (현재: ${stats.lectureHours}시간 / 필요: ${MIN_HOURS_FOR_PROMOTION}시간)`, 400, 'NOT_ELIGIBLE');
        }
        return {
...
exports.default = new InstructorService();
