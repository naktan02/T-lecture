...
describe('Assignment API Integration Test', () => {
  ...
  const endDateStr = new Date(new Date().setDate(tomorrow.getDate() + 1))
    .toISOString()
    .split('T')[0]; // 필터링 종료일 (모레)
  ...
  before(async () => {
    ...
    await prisma.instructorVirtue.deleteMany(); // 강사가능덕목 먼저 삭제
