// server/src/domains/metadata/metadata.service.js
...
const parseIntIdOrThrow = (raw, fieldName = 'id') => {
    ...
    if (!raw || Number.isNaN(n) || !Number.isInteger(n) || n <= 0) {
        throw new AppError(`유효하지 않은 ${fieldName} 입니다.`, 400, 'VALIDATION_ERROR', { [fieldName]: raw });
    }
    return n;
    };
...
const requireNonEmptyString = (value, fieldName) => {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new AppError(`${fieldName} 값이 필요합니다.`, 400, 'VALIDATION_ERROR', { [fieldName]: value });
    }
    return value.trim();
};
