...
interface InstructorWithAvailabilities {
  userId: number;
...
class AssignmentAlgorithm {
   ...
   */
  execute(
    units: UnitWithSchedules[],
    instructors: InstructorWithAvailabilities[],
  ): AssignmentResult[] {
    const assignments: AssignmentResult[] = [];
    const assignedSet = new Set<string>(); // 중복 배정 방지용 (instructorId_date)
    ...
    for (const unit of units) {
      ...
      for (const schedule of unit.schedules) {
        ...
        for (const instructor of instructors) {
          if (currentAssignedCount >= totalRequired) break;
